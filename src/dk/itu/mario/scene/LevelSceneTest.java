...
public class LevelSceneTest extends LevelScene {
    ...
    public LevelSceneTest(GraphicsConfiguration graphicsConfiguration,
                          MarioComponent renderer, long seed, int levelDifficulty, int type, boolean isCustom, boolean isSaved) {
        this(graphicsConfiguration, renderer, seed, levelDifficulty, type, isCustom, "player.txt");
    ...
    public void init() {
        ...
        if (level == null)
            if (isCustom) {
                CustomizedLevelGenerator clg = new MyLevelGenerator();
                if (isSaved) {
                    clg = new DeterministicLevelGenerator();
                }
        ...
        if (!isCustom && recorder == null)
            ...
            //BETTER VERSION
        else if (isCustom && recorder == null)
            recorder = new DataRecorder(this, (RandomLevel) level, keys);
